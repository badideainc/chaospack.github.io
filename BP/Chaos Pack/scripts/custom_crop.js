...
const BlockCustomCropGrowthComponent = {
    onRandomTick({ block }) {
        ...
        const growth = block.permutation.getState("chaospack:growth");
        block.setPermutation(block.permutation.withState("chaospack:growth", growth + 1));
    },
    onPlayerInteract({ block, dimension, player }) {
        ...
        if (player.getGameMode() === GameMode.creative) {
            // Grow crop fully
            block.setPermutation(block.permutation.withState("chaospack:growth", 7));
        } else {
            let growth = block.permutation.getState("chaospack:growth");

            // Add random amount of growth
            growth += randomInt(1, maxGrowth - growth);
            block.setPermutation(block.permutation.withState("chaospack:growth", growth));
